...
```{r}
d_fits <- d_fits |> 
  mutate(mm = map(.x = formula, 
                  .f = \(.x) model.matrix(data = d, object = .x)),
         d_pred = map(.x = predictors, 
                      .f = \(.x) d_pred |> select(mass_std1:str_c("mass_std", .x))))
  ...
  mutate(stan_data = pmap(.l = list(mm, d_pred, formula), 
                          .f = function(.x, .y, .z) {
                            d |> 
                              ...
                              compose_data(X = .x,
                                           Xpred = model.matrix(data = .y, object = .z),
                                           k = ncol(.x))
                          }))
...
d_fits_subset <- d_fits_subset |> 
  mutate(mm = map2(
    .x = row,
    .y = formula,
    .f = \(.x, .y) model.matrix(data = d |> filter(row_number() != .x), object = .y)))
...
```{r}
d_fits_subset <- d_fits_subset |> 
  mutate(d_pred = map(.x = predictors, 
                      .f = \(.x) select(d_pred, mass_std1:str_c("mass_std", .x)))) |> 
  mutate(stan_data = pmap(.l = list(row, mm, d_pred, formula), 
                          .f = function(.a, .b, .c, .d) {
                            d |> 
                              ...
                              select(brain_std:mass_std6) |> 
                              compose_data(X = .b,
                                           Xpred = model.matrix(data = .c, object = .d),
                                           k = ncol(.b))
                          }))
...
d_fits_subset <- d_fits_subset |> 
  mutate(sampling = map(.x = stan_data, 
                        .f = \(.x) sampling(object = stan_dso, data = .x, seed = 7)))
```
...
### Scoring the right data. {#sec-Scoring-the-right-data}

Since our `stan()` models from above do not have log-likelihood values saved from each, we have two options to make an alternative to McElreath's R code 7.15:
