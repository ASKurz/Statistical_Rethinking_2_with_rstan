...
Note how with the `pars` argument, I restricted the output to the first and last intercept. You can adjust that code to inspect more or fewer of the intercepts, as desired.
...
\color{blue}{\bar \alpha} & \color{blue} \sim \color{blue}{\operatorname{Normal}(0, 1.5)} \\
\color{blue} \sigma       & \color{blue} \sim \color{blue}{\operatorname{Exponential}(1)},
\end{align*}
$$
...
\color{blue}{\gamma_j} & \color{blue} \sim \color{blue}{\operatorname{Normal}(0, \sigma_\gamma), \;\;\; \text{for } j = 1, \dots, 6} \\
\bar \alpha   & \sim \operatorname{Normal}(0, 1.5) \\
\sigma_\alpha & \sim \operatorname{Exponential}(1) \\
\color{blue}{\sigma_\gamma} & \color{blue} \sim \color{blue}{\operatorname{Exponential}(1)}.
\end{align*}
$$
...
Like in the text, this model returned warnings about divergent transitions, which isn't great. As we learned back in @sec-Non-identifiable-parameters, we can check for the HMC diagnostic warnings with the `check_hmc_diagnostics()` function.

```{r}
check_hmc_diagnostics(m13.4)
```
...
bind_rows(spread_draws(m13.4, b[j]), 
  ...
  scale_color_viridis_d(option = "D", end = 0.4) +
  xlab("posterior")
```
...
If you check the `check_hmc_diagnostics()` output, you'll see we still have several divergent transitions with `m13.6`.

```{r}
check_hmc_diagnostics(m13.6)
```
...
This is something of a version of McElreath's Figure 13.5a.
...
where $x$ is now the product of two independent distributions, $v$ and $z$. For our `stan()`-based workflow, this means defining `v` and `z` in the `parameters` block, and then defining `x` as a function of the two in the `generated quantities`. But because `x` is a deterministic function of `v` and `z`, only `v` and `z` get assigned priors in the `model` block.
...
We can just use the `sampling()` to revisit `m13.4`. This time, we increase the `adapt_delta` parameter in a list fed to the `control` argument.
...
Rather than using the full `check_hmc_diagnostics()` function like above, we can get a more divergent-transition-focused perspective with the `check_divergences()` function. Here we compare the results from both `m13.4` and `m13.4b`.

```{r}
check_divergences(m13.4)
check_divergences(m13.4b)
```
...
\beta_j  & \sim \operatorname{Normal}(0, 0.5), \;\;\; \text{for } j = 1, \dots, 4 \\
\color{blue}{x_j} & \color{blue} \sim \color{blue}{\operatorname{Normal}(0, 1)} \\
\color{blue}{z_j} & \color{blue} \sim \color{blue}{\operatorname{Normal}(0, 1)} \\
\bar \alpha   & \sim \operatorname{Normal}(0, 1.5) \\
...
bind_rows(
  summarise_draws(m13.4, ess_bulk, ess_tail) |> filter(variable %in% parameter_vec),
  summarise_draws(m13.4nc, ess_bulk, ess_tail) |> filter(variable %in% parameter_vec)
) |> 
...
If you are using by-hand approach for computing model-based expectations for a multilevel model of this kind, `spread_draws()` seems like a good solution for a tidy workflow. Here's what that can look like for `actor == 2`, in `block == 1`. We just display the results in a plot resembling Figure 13.7a.
...
m13.4 |>
  ...
  facet_grid(actor ~ block, labeller = label_both)
```
...
To simulate actors for the left panel of Figure 13.7, our basic data-wrangling workflow will be similar to what we just did. This time we'll use the `ndraws` argument within `spread_draws()` to randomly sample a subset of the 4,000 post-warmup posterior draws in the posterior, rather than use them all. When we plot, instead of summarizing the results with `stat_lineribbon()` we instead just plot individual lines with `geom_line()`, and we used the `.draw` index as a stand-in for simulated actors.
