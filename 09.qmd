...
Wrangle the data into a list with the `compose_data()` function. McElreath called his dat list `dat_slim`. Here we'll follow the conventions from earlier chapters and call the object `stan_data`.
...
As to the diagnostic statistics, you can compute the $\widehat R$ and effective-sample-size statistics with the `posterior::summarise_draws()` function, particularly with the nice `default_convergence_measures()` helper.
