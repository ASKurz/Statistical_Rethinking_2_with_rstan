...
where the `_std` suffix indicates the variables are standardized (i.e., zero centered, with a standard deviation of one). 
...
```{r}
print(m5.2, pars = c("b0", "b1", "sigma"), probs = c(0.05, 0.945))
```
...
I generally do not like this style of notation, and I rarely use it. It's not common to see it in papers in my field, and I suspect it many of my colleagues would find it actively discouraging. But this style of notation will come in handy for some of the `stan()` code to come, starting with the models in @sec-A-second-method.
...
```{r}
print(m5.3, pars = c("b0", "b1", "b2", "sigma"),  probs = c(0.05, 0.945))
```
...
```{r}
print(m5.1_sim, probs = c(0.05, 0.945))
```
...
```{r}
print(m5.4, probs = c(0.05, 0.945))
```

Now we have a series of `mu[i]` rows in the summary. These also show up in the `as_draws_df()` output.
...
```{r}
print(m5.4b, pars = c("b0", "b1", "sigma"), probs = c(0.05, 0.945))
```
...
d_spur <- tibble(
  x_real = rnorm(n = n),                 # x_real as Gaussian with mean 0 and SD 1 (i.e., the defaults)
  x_spur = rnorm(n = n, mean = x_real),  # x_spur as Gaussian with mean = x_real
  y =      rnorm(n = n, mean = x_real))  # y as Gaussian with mean = x_real
```
...
```{r}
print(m5.0_spur, probs = c(0.05, 0.945))
```
...
```{r}
print(m5.3_a, probs = c(0.05, 0.945))
```
...
d <- d |> 
  mutate(k = (kcal.per.g - mean(kcal.per.g)) / sd(kcal.per.g), 
         n = (neocortex.perc - mean(neocortex.perc, na.rm = T)) / sd(neocortex.perc, na.rm = T), 
...
```{r}
print(m5.5_draft, probs = c(0.05, 0.945))
```
...
```{r}
print(m5.5, probs = c(0.05, 0.945))
```

Figure 5.9, upper left
...
Define the two `model_code`s. Notice how we've used the `n_sex` value in the first.
...
```{r}
print(m5.8, probs = c(0.05, 0.945))
print(m5.8b, probs = c(0.05, 0.945))
```
...
Define the two `model_code`s. Note our use of the `n_clade_id` value in the first.
...
```{r}
print(m5.9, probs = c(0.05, 0.945))
print(m5.9b, probs = c(0.05, 0.945))
```
...
This is a good place to introduce `tidybayes::spread_draws()` function, which can be handy for extracting posterior draws in a long format. When you have a model that uses an indexed parameter, such as the `a` parameter in `m5.9`, you can request the draws for each of those indices using `[]` notation. Note below how we have indicated we want those indices named `j` with our `a[j]` syntax. We could have also called them something more descriptive like `clade_id`, or more generic like `index`.
...
```{r}
print(m5.10, probs = c(0.05, 0.945))
```
