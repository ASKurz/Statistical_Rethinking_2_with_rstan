...
Now we can use Kruschke's `beta_a_b_from_mean_kappa()` to find the $\alpha$ and $\beta$ values corresponding to McElreath's `pbar` and `theta`.
...
If you take a look of McElreath's `m12.2`, you'll see it's a gamma-Poisson version of the non-linear model he fit in last chapter, `m11.11`. The model is
...
which is a gamma-Poisson version of the theory-based Poisson model `m11.11` from the last chapter.
...
Now we define `model_code_12.4`. I learned the basic syntax for this kind of model from [this post](https://discourse.mc-stan.org/t/vectorized-ordered-logistic-compiles-but-then-does-not-sample-correctly/10843) on the Stan forums. Note how we're adding a `generated quantities` block for the pp-check.
